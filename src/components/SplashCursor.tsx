...
export default function SplashCursor({
  ...
  // --- FOR TESTING: Use a bright white background ---
  BACK_COLOR = { r: 1, g: 1, b: 1 },
  TRANSPARENT = false // --- FOR TESTING: Set to false ---
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    console.log("Canvas initialized, starting WebGL setup...");
    ...
    function compileShader(type: number, source: string, keywords: string[] | null = null): WebGLShader | null {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        console.trace('Shader compile error:', gl.getShaderInfoLog(shader));
      }
      return shader;
    }
    ...
    function createProgram(vertexShader: WebGLShader | null, fragmentShader: WebGLShader | null): WebGLProgram | null {
      ...
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        console.trace('Program link error:', gl.getProgramInfoLog(program));
      }
      return program;
    }
    ...
    function updateFrame() {
      console.log("updateFrame running..."); // Debug log
    ...
    function handleFirstMouseMove(e: MouseEvent) {
      ...
      const posY = scaleByPixelRatio(e.clientY);
      const color = generateColor();
      updateFrame(); // Start the loop on first interaction
    ...
    return () => {
      // You can cancel the animation frame here if you track its ID.
      // For simplicity, we'll leave it running.
    };
